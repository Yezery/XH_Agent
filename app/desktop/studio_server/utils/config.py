...
class Config:
    ...
    def __init__(self, properties: Dict[str, ConfigProperty] | None = None):
        ...
        self.ensure_default_settings()  # 确保默认配置存在
    ...
    def __getattr__(self, name: str) -> Any:
        # 如果属性名为"_properties"，则调用父类的__getattribute__方法获取属性值
        if name == "_properties":
            return super().__getattribute__("_properties")
        # 如果属性名不在_properties中，则调用父类的__getattribute__方法获取属性值
    ...
    def settings_path(cls, create=True):
        settings_dir = os.path.join(Path.home(), ".XHAgentData")
    ...
    def load_settings(cls):
        # 检查settings_path文件是否存在，如果不存在则返回空字典
        if not os.path.isfile(cls.settings_path(create=False)):
            return {}
        # 打开settings_path文件，并使用yaml.safe_load方法读取文件内容
    ...
    def update_settings(self, new_settings: Dict[str, Any]):
        ...
        with threading.Lock():
            ...
            with open(self.settings_path(), "w",encoding="utf-8") as f:
                yaml.dump(current_settings, f,allow_unicode=True)
            self._settings = current_settings

    def ensure_default_settings(self):
        """如果 settings.yaml 不存在，则创建并写入默认配置"""
        settings_path = self.settings_path()
        if not os.path.exists(settings_path):  # ❌ 文件不存在
            default_settings = {
                
            }
            with open(settings_path, "w",encoding="utf-8") as f:
                yaml.dump(default_settings, f,allow_unicode=True)
            self._settings = default_settings  # 更新内存中的 settings
        else:
            self._settings = self.load_settings()  # 加载已有配置
